...
namespace lv
{
	...
	class LVDB{
	public:
		static const Bytes& KeyMin;
		static const Bytes& KeyMax;

	public:
		LVDB(){}
		virtual ~LVDB(){};
		static LVDB* open(const Options &opt);
		...
		/* meta operates */
		virtual int meta_set(const Bytes &key, const Bytes &val) = 0;
		virtual int meta_get(const Bytes &key, std::string *val) = 0;
		virtual int meta_del(const Bytes &key) = 0;
		virtual int meta_list(std::vector<std::string> *list) = 0;
		...
		virtual int set_bit(const Bytes &key, int bitoffset, int on, char log_type = BinlogType::SYNC) = 0;
		virtual int get_bit(const Bytes &key, int bitoffset) = 0;
